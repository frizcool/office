                    ...
                    <div class="col-md-6 text-uppercase">
                        @for ($i = 0; $i < count($pejabat)/2; $i++)
                            <p class="@if (in_array($pejabat[$i]->id, $disposisi_kepada)) fw-bold @endif">
                                <span class="@if (in_array($pejabat[$i]->id, $disposisi_kepada)) circle @endif">{{ $i + 1 }}</span> 
                                {{ $pejabat[$i]->jabatan }} 
                                @if (in_array($pejabat[$i]->id, $disposisi_kepada)) &#10003; @endif
                            </p>
                        @endfor
                    </div>
                    <div class="col-md-6 text-uppercase">
                ...
                <div class="col-md-4 p-1 text-uppercase">
                    @for ($a = 0; $a < count($disposisi_list)/3; $a++)
                        <p class="@if (in_array($disposisi_list[$a]->id, $disposisi_list_id)) fw-bold @endif">
                            <span class="@if (in_array($disposisi_list[$a]->id, $disposisi_list_id)) circle @endif">{{ $a + 1 }}</span> 
                            {{ $disposisi_list[$a]->ur_disposisi_list }} 
                            @if (in_array($disposisi_list[$a]->id, $disposisi_list_id)) &#10003; @endif
                        </p>
                    @endfor
                </div>
                <div class="col-md-4 p-1 text-uppercase">
